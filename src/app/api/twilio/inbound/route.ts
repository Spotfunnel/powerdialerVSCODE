    ...
    try {
        ...
        return await withPrismaRetry(async () => {
            ...
            if (!lead) {
                ...
                // Use the phone number itself as the placeholder companyName so
                // the dialer/history UI never shows the legacy "Unknown Caller"
                // sentinel for autogenerated inbound leads.
                lead = await prismaDirect.lead.create({
                    data: {
                        phoneNumber: fromNumber,
                        firstName: "Inbound",
                        lastName: "Caller",
                        companyName: fromNumber,
                        status: "NEW",
                        source: "INBOUND_CALL"
                    }
                });
            ...
            const callerLabel = (lead.firstName + " " + lead.lastName).trim() || fromNumber;
            const companyLabel = displayCompanyName({
